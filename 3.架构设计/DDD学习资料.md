...
**学习线路**
基础编程--编程进阶--类库使用--框架使用--领域建模--设计模式--架构设计--平台设计

**技术网站**
入门培训：极客学院、imooc、w3school
进阶与社区：cnblogs，csdn
架构设计：InfoQ
开源与工具：oschina
项目管理：coding、gitee
...
设计模式、框架 的意义在于，代码复用，规范化。
模式设计(Design pattern)是一套被反复使用、多数人知晓的、经过分类编目的、代码设计经验的总结。使用设计模式是为了可重用代码、让代码更容易被他人理解、保证代码可靠性。 毫无疑问，设计模式于己于他人于系统都是多赢的，设计模式使代码编制真正工程化，设计模式是软件工程的基石，如同大厦的一块块砖石一样。http://baike.so.com/doc/4379170-4585358.html
...
UnitOfWork模式 主要解决 数据持久化执行的 事务化 处理，保持数据完整。

例如，多次持久化不同记录，要保持数据完整性，出错回滚。
转账功能，2个人相互转账，一个记录执行减法后持久化，另一个记录执行加法后持久化，这个时候必须2个记录执行都没有问题才能算完成执行。如果中间出现错误，就要回滚，所以要用到事务处理。

当然不用模式的化，也可以在 数据提交之前做一个事务处理。设计模式的意义在于，代码复用，规范化。